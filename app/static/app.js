...
async function approveChecklistWithSignature(){const input=document.getElementById("checklistSignatureFile"),file=input?.files?.[0];if(!file){alert("Select Jessica's signature image first.");return;}if(!file.type.startsWith("image/")){alert("The signature must be an image.");return;}const reader=new FileReader();reader.onload=async()=>{try{currentChecklistRun=await checklistJson(`/checklists/runs/${currentChecklistRun.id}/approve`,{method:"POST",body:JSON.stringify({signature_data:reader.result})});renderChecklistEditor();alert("Report signed and approved.");}catch(e){alert(e.message);}};reader.readAsDataURL(file);}
